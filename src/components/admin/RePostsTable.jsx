              ...
              <tbody>
                {posts.map((post, index) => (
                  <tr key={index} className="border-b bg-white dark:border-neutral-500">
                    ...
                    <td className="whitespace-nowrap font-bold md:text-lg md:px-6 px-2 md:py-4">
                      <button onClick={()=>navigateTo(post.postId._id)} className="text-blue-950 hover:text-blue-600">
                        <FontAwesomeIcon icon={faEye} />
                      </button>
                    </td>
                    <td className="whitespace-nowrap font-bold me-5 ">
                      <button onClick={()=>change(post.postId._id,post.postId.block?"false":"true")} className={`${post.postId.block?"bg-green-700":"bg-red-600"} md:text-lg md:px-3 px-2 md:py-2 text-white rounded-md `}>{post.postId.block?"UnBlock":"Block"}</button>
                    </td>
                  </tr>
                ))}
              </tbody>
