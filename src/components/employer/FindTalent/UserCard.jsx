      ...
      {userData.map((user, i) => (
        ...
        >
          <div className="w-full md:w-3/4">
            ...
            <div className="mt-2">
              <span className="font-serif font- text-lg">{user.about}</span>
            </div>
            <div className="my-1">
              <span className="md:text-xl  font-bold">
                ...
                {user.skills.map((skill, i) => (
                  <span
                    key={i}
                    className="bg-gray-600 ms-1   rounded-md text-white font-medium text-base px-2"
                  >
                    {skill}
                  </span>
                ))}
            ...
            <div className="flex md:flex-col m">
              <div className="md:ms-auto">
                <button
                  ...
                  className="bg-blue-950 hover:bg-blue-800 text-white px-4 py-1 rounded-md font-semibold md:text-lg"
                >
                  Profile
                </button>
              </div>
              <div className="ms-auto md:my-2">
                <button
                  ...
                  className="bg-blue-950 hover:bg-blue-800 text-white px-6 py-1 rounded-md font-semibold md:text-lg"
                >
                  Chat
                </button>
              </div>
              <div className="ms-auto">
                {postData.invites.some(
                  (invite) => invite.userId == user._id
                ) ? (
                  <button className="bg-blue-950 hover:bg-blue-800 text-white px-5 py-1 rounded-md font-semibold md:text-lg">
                    Invited
                  </button>
                ) : (
                  <button
                    onClick={() => invite(user._id, postData._id)}
                    className="bg-blue-950 hover:bg-blue-800 text-white px-5 py-1 rounded-md font-semibold md:text-lg"
                  >
                    Invite
                  </button>
                )}
